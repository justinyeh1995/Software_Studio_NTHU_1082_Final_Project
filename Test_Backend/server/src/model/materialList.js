...
const pr =
  ...
  .then(()=>{
    return _page.open('https://lms.nthu.edu.tw/course.php?courseID=43470&f=doclist')
  })
  .then(status => {
  ...
  .then((content)=> {
        ...
        $('.tableBox > table > tbody > tr > td[align="left"]').each( function(i, elem) { 
          var link = 'https://lms.nthu.edu.tw'+$(this).find('a').attr("href");
          var materialname = $(this).text();
          var ID = $(this).find('a').attr("href").match(/\d+/g).map(Number)[1];
          materialList.push({"attachname": materialname, "link": link, "id": ID})
        })
        materialPack[0] = {"materialList": materialList};
        console.log(materialPack[0])
  })
  .then(()=> {
